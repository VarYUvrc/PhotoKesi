...
1. アプリを開くと、上下二列の整理ボードに **類似グループを 1 つだけ** 表示します。
   ...
   * 画面上部のヘッダーは「< 前へ」「現在のグループ: n / N」「次へ >」の三分割レイアウトで構成し、中央テキストは常に中央寄せになります。先頭・末尾のグループでは対応するボタンを自動で非活性化し、時間幅に関する案内は設定画面に集約します。
   ...
   * 類似グループの探索は常に「現在位置から最大 10 グループ先」までを先読みバッファとして維持し、バッファ残量が 7 を下回るとバックグラウンドで穏やかに補充します。無制限に奥まで掘り進めず、ユーザー操作と同じテンポで計算資源を使います。
   * 端末が低電力モードに入ったり温度が高くなった場合は、先読みバッファを自動的に 5 グループへ縮退し、アプリの前景／背景遷移に合わせて探索キューを一時停止・再開して端末負荷を抑えます。
...
> ポイント：**常に「現在 + 10 グループ」のバッファ**を維持し、最新の写真から優先して価値を出します。ユーザーが操作しているあいだも端末負荷を監視しながら穏やかに補充し、低電力や高温時には自動的にバッファを縮退させます。
