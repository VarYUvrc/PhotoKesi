...
このアプリは、同じ時間帯に撮られた“よく似た写真”だけを自動で束ね、**最初からベスト1枚にチェックが入った状態**で上下二段の整理ボードに並べます。上段には「残す列」としてチェック済み写真が並び、下段には「捨て候補列」として未チェックの写真が並びます。ユーザーは、**チェックが付いたものだけを残す**という単純なルールのまま、**「チェック以外をまとめて削除候補へ」**→**最後に確認して削除**まで、一息で片づけられます。
...
1. アプリを開くと、上下二列の整理ボードに **類似グループを 1 つだけ** 表示します。
   ...
   * 上段（残す列）には **ベストショット 1 枚がチェック済み** で初期配置され、サムネ右上のチェックバッジが光ります。
   * 下段（捨て候補列）には、ベスト以外の写真が未チェックで並びます。
   * 前後の撮影時刻が設定した時間幅内に連鎖して存在する写真だけが束ね対象になり、孤立した 1 枚きりの写真は整理ボードに表示されません。
   * メイン画面のサムネイルを **タップ** するとフルサイズの元写真ビューに遷移し、右上のトグルでチェックのオン・オフを切り替えられます（切り替え結果は即座に列へ反映）。
   * 元写真ビューでは左上の戻るボタンでサムネ一覧へ戻れるほか、**左右スワイプ** で同じ束の別写真に移動し、**上方向へのスワイプ** でメイン画面へ戻ります。ピンチ操作は拡大に利用できます。
   * メイン画面上でもサムネイルのチェックバッジをタップする、または **上下方向のスワイプ** で列を入れ替え操作が可能です。
   * 右上の歯車アイコンから設定へ移動し、グルーピング時間幅（15〜240分・15分刻み）をその場で調整できます。
   * 有料プランでは、上段のチェック済み写真に **理由バッジ**（例：手ぶれが少ない、露出が安定している、被写体が中心にある）を表示します。

2. 上下二列のすぐ下に配置した **ゴミ箱アイコン付き「チェック以外をバケツへ」ボタン** を押すと、
   そのグループ内の **未チェック写真** をまとめて **削除候補（バケツ）** に送ります。
   * バケツ画面では写真がすべて未チェック状態で並び、誤って送った写真は **上方向のスワイプ** でメイン画面に戻せます（戻した写真は再度列に配置なおします）。
