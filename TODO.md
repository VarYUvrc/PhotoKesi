...
- ビルド確認: README手順でのビルド成否を点検
    -  README記載のスクリプトを実行してビルドログを取得
    -  出力ログから修正すべき事項を洗い出す
...
* [x] B2: 類似グループ探索を「現在位置 + 最大 10 グループ」のバッファで段階化する。
    ...
    * [x] B2-6: Liquid Glass を活かすため、メイン画面に Apple らしい自然なグラデーション背景を導入する（ライト／ダーク対応・可読性確保・パフォーマンス配慮）。
...
* [x] B4: 一度「残す」と確定した写真に永続フラグを付け、再探索を防ぐ。
    * [x] B4-1: `PhotoRetentionStore`（仮）を設計し、写真の `localIdentifier` とハッシュ署名を保存できる永続層を用意する。
    * [x] B4-2: 仕分け確定時に残す写真を永続化し、起動直後のサムネイル収集とグルーピングでフラグ済み写真を除外する。
    * [x] B4-3: 設定画面にフラグのリセット導線（確認ダイアログ付き）を追加し、再読み込みでフラグ解除後の状態に反映する。

### フェーズC：精度と安心を上げる

* [x] C1: 類似判定を高速二段構成に刷新し、多様な被写体へ対応する。
    * [x] C1-1: 96px縮小サムネから aHash／dHash／pHash を同時計算し、しきい値をチューニングする。
    * [x] C1-2: LAB12binヒストグラムとエッジ方向ヒストグラムを加え、シーンタイプに応じて重みを切り替える。
* [x] C1-3: 閾値境界のグループで LAB／エッジ指標の補強判定を計測し、スループットを確認する。
* [x] C1-4: デバッグ用の類似判定チューニングUIを設定画面に追加し、閾値へ反映する。
    * [x] C1-4-1: ViewModelにデバッグ調整用パラメータと再構築ロジックを追加する。
    * [x] C1-4-2: 設定画面に調整スライダー／トグルとリセット導線を実装する。
    * [x] C1-4-3: 調整変更時にグループ構成へ即時反映されることを確認する。
* [x] C1-5: 初期グループを即時表示し、残りのグループ探索はバックグラウンドで補充する。
    * [x] C1-5-1: ロード処理を第1グループ表示とバッファ補充に分離する。
    * [x] C1-5-2: スケルトンが必要最小限で消えることを確認する。
* [ ] C2: ベスト判定を写真タイプ別スコアに再設計し、ポートレート／風景／料理／自撮りを最適化する。
    * [ ] C2-1: 顔検出・彩度・輝度勾配から写真タイプを推定し、ViewModelにキャッシュする。
    * [ ] C2-2: シャープネス・露出・ノイズ・構図中心度の重みをタイプ別に再定義し、スコア計算を高速化する。
    * [ ] C2-3: ベスト選定理由テキストをタイプ別テンプレートで生成し、将来のローカライズを考慮する。
* [ ] C3: 「バケツ送り済み」サムネイル状態を追加し、タップで復活できるようにする。
    * [ ] C3-1: グレーアウト＋中央バッジのスタイルを設計し、アクセシビリティを検証する。
    * [ ] C3-2: バケツ戻し操作で仕分け済フラグを即復元し、履歴スタックと同期させる。
    * [ ] C3-3: バケツ状態の一貫性をユニットテストし、戻る／進む操作と競合しないようにする。
* [ ] C4: 理由吹き出し UI を実装し、ベスト写真の上に理由を表示する。
    * [ ] C4-1: グループ表示時に吹き出しコンポーネントを表示／非表示できるようにする。
    * [ ] C4-2: フルスクリーン表示でも理由を参照できるよう、アクセシビリティラベルを付与する。
    * [ ] C4-3: テキスト量に応じてレイアウトを自動調整し、多言語対応を視野に入れる。
* [ ] C5: 「残す」フラグの再出現防止を知覚ハッシュ照合で堅牢化し、誤復活を防ぐ。
