...
* [ ] A6: **メイン画面** に上下二列レイアウトでグループを表示し、上段は残す列（初期状態でベストショット1枚）、下段は捨て候補列とする。**チェック＝残す**を実装。
    * [ ] A6-1: 上段の残す列にベストショット1枚を固定し、サムネ右上のチェック表示を保つ。
    * [ ] A6-2: 下段の捨て候補列にその他の写真を並べ、上下スワイプまたはチェック操作で列間移動させる。
    * [ ] A6-3: 上下段の直下に「ゴミ箱アイコン, "チェック以外をバケツへ"」ボタンを配置する。
    * [ ] A6-4: グループ生成やサムネイル取得が完了するまで、上下二列構成に合わせたスケルトンプレースホルダを表示する（`redacted` や単色矩形を活用し、追加素材は用いない）。
    * [ ] A6-5: サムネイルをタップした際にフルサイズビューを表示し、右上トグルでチェックを切り替えられるようにする（左上にメイン画面へ戻るボタンを配置）。
    * [ ] A6-6: フルサイズビューで左右スワイプによる画像切替と上方向スワイプでメイン画面へ戻るジェスチャを実装する。
* [ ] A7: **「チェック以外をバケツへ」** ボタン（上下段の直下）と **「バケツを空にする」→削除確認モーダル→削除確定** のフローを実装。
    * [ ] A7-1: バケツ画面で未チェックの写真を上方向スワイプでメイン画面へ戻すリカバリ操作を実装し、初期状態を未チェックで揃える。
